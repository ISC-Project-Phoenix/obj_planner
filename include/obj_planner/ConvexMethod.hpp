...
public:
    ...
    virtual LeftRightResults classify(const std::vector<cv::Point>& convex_hull,
                                      const std::vector<cv::Point>& detections_2d) = 0;
...
public:
    LeftRightResults classify(const std::vector<cv::Point>& convex_hull,
                              const std::vector<cv::Point>& detections_2d) override;
};
...
private:
    /// Set detections_2d
    std::vector<cv::Point> get_detections_vector(const geometry_msgs::msg::PoseArray& detections_array);
